...
"""
文言文实词抽题程序
功能：加载 Excel 资源文件，通过 GUI 生成三种输出：
  1) 输出 docx 题目（带答案）
...
def stratified_sample(data_in_range, weights, count, order_mode):
    ...
    返回: list of (sentence, word, meaning, difficulty)
    """
    by_diff = {1: [], 2: [], 3: [], 4: [], 5: []}
...
class App:
    def __init__(self, root):
        ...
        frm_mode = tk.LabelFrame(root, text="选择模式（三选一）", font=("宋体", 10, "bold"))
    ...
    def on_mode_change(self, *args):
        for f in [self.frm_docx_q, self.frm_pptx_q, self.frm_docx_h]:
            f.pack_forget()
