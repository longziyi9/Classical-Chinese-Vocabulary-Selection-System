...
def generate_pptx_questions(data_rows, sub_cfg):
    ...
    for paper_idx in range(1, num_papers + 1):
        ...
        for chunk in ans_chunks:
            ...
            for j_in_page, (sent, word, meaning, *_) in enumerate(chunk, 1):
                global_ans_idx += 1
                p = tf_a.paragraphs[0] if j_in_page == 1 else tf_a.add_paragraph()
...
def generate_ability_test(data_rows, sub_cfg):
    ...
    for idx, item in enumerate(all_items, 1):
        p = doc.add_paragraph()
        p.paragraph_format.line_spacing = 1.0
        text = f"{idx}.{item['sentence']}（{item['row_id']}）    {item['word']}:{'_' * 4}"
        run = p.add_run(text)
        set_cn_font(run, "宋体", 10.5)  # 五号 = 10.5pt
