...
def stratified_sample(data_in_range, weights, count, order_mode):
    ...
    for d in data_in_range:
        ...
        for ex in d["examples"]:
            by_diff[diff].append((ex["sentence"], d["word"], d["meaning"], diff))
...
def simple_sample(data_in_range, count, order_mode):
    ...
    for d in data_in_range:
        for ex in d["examples"]:
            all_items.append((ex["sentence"], d["word"], d["meaning"], d["difficulty"]))
...
def generate_pptx_questions(data_rows, sub_cfg):
    ...
    for paper_idx in range(1, num_papers + 1):
        ...
        for chunk in q_chunks:
            ...
            for idx_in_page, (sent, word, meaning, _) in enumerate(chunk, 1):
                global_idx += 1
                line = f"{global_idx}. {sent}    {word}:{'_' * 4}"
                p = tf_q.paragraphs[0] if idx_in_page == 1 else tf_q.add_paragraph()
                run = p.add_run()
                run.text = line
                set_pptx_run_font(run, "华文中宋", 24)

        # ---- 答案页（每页10条） ----
        global_ans_idx = 0  # 全局答案编号
        ...
        for chunk in ans_chunks:
            ...
            for j, (sent, word, meaning, _) in enumerate(chunk, 1):
                ...
                run1 = p.add_run()
                run1.text = f"{global_ans_idx}. {sent}    {word}:"
...
# ============================================================
# 模式四：能力测试模式（v1.1 新增，v1.3 修改下划线为16字符）
# ============================================================

def generate_ability_test(data_rows, sub_cfg):
    """能力测试模式：指定范围内所有例句打乱输出，下划线16字符"""
    try:
        start_id = int(sub_cfg["start"].get())
        end_id = int(sub_cfg["end"].get())
    except ValueError:
        messagebox.showerror("错误", "请输入有效的整数序号范围")
        return
    ...
    for d in data_range:
        for ex in d["examples"]:
            all_items.append((ex["sentence"], d["id"], d["word"]))

    if not all_items:
        messagebox.showerror("错误", "所选范围内没有例句")
        return
    ...
    for idx, (sent, orig_id, word) in enumerate(all_items, 1):
        # 格式：[序号]. [例句]（[总序]）    [实词]:________________
        line = f"{idx}. {sent}（{orig_id}）    {word}:{'_' * 16}"
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("文言文实词抽题系统 v1.3")
